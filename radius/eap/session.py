...
class EAP:
    ...
    async def on_eap(self, request, response):
        ...
        key = (message[1], request['NAS-IP-ADDRESS'], *request.remote)
        sess = self.session_cache.pop(key, None) or EAP_Session(self.ctx, self.method)
        key = ((message[1]+1) % 256, request['NAS-IP-ADDRESS'], *request.remote)
        ...
        if sess.on_user.done():
            userdata = self.on_framed(request, response, await sess.on_user)
