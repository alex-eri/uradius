...
def get_new_key_from_sha(start_key, session_key, length=16):
    sha = hashlib.sha1()
    sha.update(start_key[:length])
    sha.update(SHApad1)
    sha.update(session_key[:length])
    sha.update(SHApad2)
    InterimKey = sha.digest()
    return InterimKey[:length]
...
def master_gen_keys(master_key, length=16):
    ...
    master_send_key = get_asymetric_start_key(master_key, length, True, True)
    master_recv_key = get_asymetric_start_key(master_key, length, False, True)
